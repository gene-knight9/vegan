...
`permutest.cca2` <-
            ...
            strata = NULL, parallel = getOption("mc.cores"), ...)
  {
    ...
    # 保留小数位数，这里假设保留 3 位小数
    digits <- 10
    F.0 <- round(F.0, digits)
    F.perm <- round(F.perm, digits)
    chi <- round(c(Chi.z, Chi.xz), digits)
    num <- round(num, digits)
    den <- round(den, digits)
    df <- round(c(q, r), digits)

    sol <- list(call = Call, testcall = x$call, model = model,
                F.0 = F.0, F.perm = F.perm,  chi = chi,
                num = num, den = den, df = df, nperm = nperm,
